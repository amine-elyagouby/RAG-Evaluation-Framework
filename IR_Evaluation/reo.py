...
sim_retriever = faiss_index.as_retriever(search_kwargs={"k": 10})
...
for d in tqdm(data, total=len(data), desc="Processing", position=0, leave=True):
    ...
    ir_results_1 = sim_retriever.get_relevant_documents(query)
    ir_results = reordering.transform_documents(ir_results_1)
...
with open('results_dev_reo_1.json', 'w') as json_file:
